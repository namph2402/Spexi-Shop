...
@section('content')
    ...
    <div class="container-fluid">
        @if (count($bannerPromotions) > 0)
            <div class="row px-xl-5 pb-5">
                <div class="col-12 banner-promotion">
                            ...
                            @foreach ($bannerPromotions as $key => $b)
                                @if ($key == 0)
                                    <div class="carousel-item position-relative active" style="height: 430px;">
                                        <img class="position-absolute w-100 h-100" data-src="{{ $b->image }}"
                                            src="{{ $b->image }}" style="object-fit: cover;">
                                    </div>
                                @else
        ...
        <div class="row px-xl-5 pb-5">
            <div class="bg-light list-promotion text-uppercase">
                <li class="promotion-item">
                    <span class="promotion-link promotion-active">Tất cả</span>
                </li>
        ...
        <div class="row px-xl-5">
            ...
            <form action="{{ Request::url() }}" class="col-lg-3 col-md-4 mb-3 formSearchP wow fadeIn" data-wow-delay="0.1s"
                id="formSearchP">
                ...
                <div class="search-price mb-30">
                    <div class="group">
                        ...
                        <div class="range-input">
                            <input class="range-min" name="priceFrom" max="1000000" step="10000" type="range"
                                value="{{ request()->priceFrom ? request()->priceFrom : 0 }}">
                            <input class="range-max" name="priceTo" max="1000000" step="10000" type="range"
                                value="{{ request()->priceTo ? request()->priceTo : 1000000 }}">
                        </div>
                        <div class="range-text">
                            <div class="text-min">{{ request()->priceFrom ? request()->priceFrom : 0 }}</div>
                            <div class="text-max">
                                {{ request()->priceTo ? number_format(request()->priceTo, 0, '.', '.') : number_format(1000000, 0, '.', '.') }}
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="search-color">
                        ...
                        <div class="bg-light mb-30 search-group">
                            <div class="row pl-3 pr-3 list-item-search">
                                @foreach ($colors as $c)
                                    <div
                                        class="col-md-6 custom-control custom-checkbox d-flex align-items-center justify-content-between mb-3">
                                        <input type="checkbox" class="custom-control-input colorItem"
                                            value="{{ $c->id }}" id="{{ $c->name }}"
                                            @if (in_array($c->id, $arrColor)) checked @endif>
                        ...
                        <div class="bg-light mb-30 search-group">
                            <div class="row pl-3 pr-3 list-item-search">
                                @foreach ($sizes as $s)
                                    <div
                                        class="col-md-6 custom-control custom-checkbox d-flex align-items-center justify-content-between mb-3">
                                        <input type="checkbox" class="custom-control-input sizeItem"
                                            value="{{ $s->id }}" id="{{ $s->name }}"
                                            @if (in_array($s->id, $arrSize)) checked @endif>
                                        <label class="custom-control-label"
                            ...
                            <div class="col-lg-4 col-md-6 col-sm-6 pb-1 product wow fadeInUp" data-wow-delay="0.1s">
                                <div class="product-item bg-light mb-4">
                                    <div class="product-img position-relative overflow-hidden">
                                        <img class="img-fluid w-100" data-src="{{ $p->image }}"
                                        ...
                                        <div class="d-flex align-items-center justify-content-center mt-2">
                                            @if ($p->sale_price == $p->price)
                                                <h5 class="product-text-price price-sale">
                                                    {{ number_format($p->price, 0, '.', '.') }}đ</h5>
                                            @else
                                                <h5 class="product-text-price price-sale">
                                                    {{ number_format($p->sale_price, 0, '.', '.') }}đ</h5>
